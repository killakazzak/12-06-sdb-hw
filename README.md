# Домашнее задание к занятию "`Репликация и масштабирование. Часть 1`" - `Тен Денис`
...
План выполнения горизонтального и вертикального шаринга базы данных включает разделение таблиц между серверами, создание отдельных баз данных на каждом сервере, настройку репликации данных и определение режимов работы серверов. 
Сервера будут работать в режиме чтения и записи, обрабатывая запросы на добавление, изменение и удаление данных.
Блоксхема визуализирует структуру системы и описывает режимы работы серверов, что бы можно было понять, как они будут обрабатывать запросы.

![image](https://github.com/killakazzak/12-06-sdb-hw/assets/32342205/c4bb3012-cb88-4ec7-928f-f250c2c570ce)
...
Главное преимущество этого метода заключается в обеспечении высокой доступности системы. Если master-сервер выходит из строя, slave-сервер может автоматически стать активным и обслуживать запросы клиентов без прерывания работы системы. Этот метод также обеспечивает отказоустойчивость данных. Изменения, вносимые на master-сервере, реплицируются на slave-сервер, что позволяет создать резервную копию данных. Кроме того, данный подход позволяет распределить нагрузку на несколько серверов. Slave-серверы могут обрабатывать запросы клиентов параллельно, что увеличивает производительность системы.

### Master-сервер и несколько slave-серверов:

Одним из основных преимуществ этого метода является возможность горизонтального масштабирования системы. При увеличении нагрузки можно просто добавить новые slave-серверы, которые будут обрабатывать запросы клиентов параллельно. Этот подход также обеспечивает отказоустойчивость. Если один из slave-серверов выходит из строя, остальные серверы могут продолжать обслуживать запросы клиентов. Распределение нагрузки между slave-серверами также позволяет повысить производительность системы.

### Активный сервер со специальным механизмом репликации — distributed replicated block device (DRBD):

Одно из главных преимуществ этого метода - гарантия согласованности данных между активным сервером и его репликами. DRBD обеспечивает синхронизацию данных в реальном времени между серверами, что позволяет избежать потери данных в случае сбоя. Этот метод также обеспечивает высокую доступность системы. Если активный сервер выходит из строя, одна из его реплик может автоматически стать активной и продолжать обслуживать запросы клиентов. DRBD позволяет использовать реплики для распределения нагрузки и увеличения производительности системы.

### SAN-кластер:

Одно из главных преимуществ SAN-кластера - централизованное хранение данных. Все серверы в кластере могут обращаться к общему хранилищу данных, что облегчает управление и обеспечивает единообразный доступ к данным. SAN-кластер обеспечивает отказоустойчивость данных. Если один из серверов выходит из строя, остальные серверы могут продолжать работать с данными, сохраненными на общих хранилищах. Этот метод позволяет масштабировать хранилище данных при необходимости, добавляя новые диски или расширяя существующие. Это обеспечивает гибкость и увеличивает емкость системы.

По опыту SAN-кластер самый надежный варинат (если позволяют финансовые возможности)
---
